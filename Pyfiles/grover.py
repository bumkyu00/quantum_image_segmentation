...
def countBitstring(count):
    ...
    for i in range(2**nqubits):
        if format(i, '#012b')[2:12] in count:
            f.append(count[format(i, '#012b')[2:12]])
        else:
            f.append(0)
    return np.array(list(f))

def countBitstringArea(count):
    sum = 0
    for i in range(2**nqubits):
        if format(i, '#012b')[2:12] in count and count[format(i, '#012b')[2:12]] < 75:
            y = i // 32
            x = i % 32
#             if x >= 15 and x <= 20 and y >= 20 and y <= 25:
            sum += 1
    return sum

def imigize1(count):
    countlist=countBitstring(count)
    out_img=np.array(list(partition(32, countlist)))/max(countlist);
