...
def extractRed(i,path,name, pixel):
    img = Image.open(
        path+str(name)+str(i)+'.jpg')
    red, green, blue = img.split()
    y1 = np.asarray(red.getdata(), dtype=np.float64).reshape((red.size[1], red.size[0]))
    data = np.asarray(y1, dtype=np.uint8)
    image = Image.fromarray(data,'L')
    image= image.resize((pixel, pixel))
    image=np.asarray(image.getdata(), dtype=np.float64).reshape((image.size[1], image.size[0]))
    image=np.asarray(image, dtype=np.uint8)    
...
def imageBinarize(data, threshold):
